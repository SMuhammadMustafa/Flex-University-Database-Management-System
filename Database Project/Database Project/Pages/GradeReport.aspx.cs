    ...
    public partial class GradeReport : System.Web.UI.Page
    {
        ...
        protected void PopulateCourses()
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                {
                    courseDDL.DataSource = dataTable;
                    courseDDL.DataTextField = "CNAME"; 
                    courseDDL.DataValueField = "CNAME"; 
                    courseDDL.DataBind();

                    courseDDL.Items.Insert(0, new ListItem("Select an item", ""));
                }
            }
        }
        ...
        {
            string courseLabel = courseDDL.SelectedItem.Text;
            string courseStr = "";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                string query = "SELECT STUDENTID, U.NAME, S.SECTIONNAME, GRADES.COURSECODE, GRADE FROM GRADES INNER JOIN COURSETEACHING CT ON GRADES.COURSECODE = CT.COURSECODE  INNER JOIN SECTIONS S ON CT.SECTIONID = S.SECTIONID INNER JOIN USERS U ON GRADES.STUDENTID = U.USERID WHERE GRADES.COURSECODE = @courseDDL AND TEACHERID = @username";

                SqlCommand command = new SqlCommand(query, connection);

                command.Parameters.AddWithValue("@courseDDL", courseStr);
                command.Parameters.AddWithValue("@username", userId);
